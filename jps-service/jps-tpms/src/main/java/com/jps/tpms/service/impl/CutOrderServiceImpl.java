...
//		this.saveBatch(list1);
		...
		Map<Long, WorkOrderDo> collect = workOrderDos.stream().collect(Collectors.toMap(WorkOrderDo::getId, e -> e));

		List<CutOrderVo> list2 = records.stream().map(e -> {

			WorkOrderDo workOrderDo = collect.get(e.getWorkOrderId());
